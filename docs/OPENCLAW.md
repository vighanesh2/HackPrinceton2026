# OpenClaw + Make It Make Sense

This branch adds the **`makeitmakesense`** OpenClaw skill under `openclaw/skills/makeitmakesense/`. It calls your local Next.js app’s platform APIs (`/api/platform/agent`, consistency, decision log). **No secrets live in the skill** — use environment variables and `~/.openclaw/openclaw.json` on each machine.
...
1. **Skill folder** (from this repo) → OpenClaw workspace, e.g.  
   `~/.openclaw/workspace/skills/makeitmakesense/`  
   (same layout: `SKILL.md`, `makeitmakesense.js`.)
...
3. **Supabase session** — authenticated API routes expect a **browser session cookie**. Set one of:
   - `MAKEITMAKESENSE_COOKIE` — full `Cookie` header value after you sign in (DevTools → Application → Cookies, or Network → request headers).
   - Legacy alias: `RONTZEN_COOKIE` (same shape).

4. **Base URL** (optional) — `MAKEITMAKESENSE_BASE_URL` (default `http://localhost:3000`).

5. **OpenClaw config** — in `~/.openclaw/openclaw.json`, under the skill entry for `makeitmakesense`, set `env` for the variables above. **Do not** commit that file; it is machine-local.
